...
const Shop = () => {
  ...
  const handleFilter=ele=>{
      const filterValue=ele.target.value
      if(filterValue==='sofa'){
  ...
  const handleSearch=ele=>{
    const searchTerm=ele.target.value
    const searchedProduct=products.filter(item=>item.productName.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase()))
          ...
          <Col lg='3' md='3'>
            <div className="filter__widget">
              <select onClick={handleFilter}>
                <option>Choose item by Category</option>
